...
class TestLineItemImportStructure:
    ...
    TESTDATA = os.path.join(BASE_DIR, "test-data")

    def test_drmis_data_folder_exists(self):
        assert os.path.exists(self.TESTDATA)
    ...
    def test_is_not_dnd_cost_center_report(self):
        fname = os.path.join(self.TESTDATA, "encumbrance_errors.txt")
    ...
    def test_clean_header(self):
        ...
        assert len(er.data["header"]) == er.COLUMNS - 2

    def test_find_header_line_returns_zero_on_failure(self):
        er = LineItemProcessor(os.path.join(self.TESTDATA, "encumbrance_no_line_header.txt"), None)
    ...
    def test_is_data_line(self):
        ...
        for line in lines:
            assert er.is_data_line(line["line"]) == line["result"]
    ...
    def test_line_to_csv_returns_list_or_Exception(self):
        ...
        assert 20 == len(result_ok)
        assert type(result_ok) is list
