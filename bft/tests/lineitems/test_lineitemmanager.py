...
class TestLineItemManager:
    ...
    def test_line_item_dataframe(self, populatedata, upload):
        df = LineItemManager().line_item_dataframe()
        assert 0 < len(df)
        assert "Lineitem_ID" in df.columns
        assert "Costcenter_ID" in df.columns

    def test_line_item_dataframe_single_fund(self, populatedata, upload):
        df = LineItemManager().line_item_dataframe(fund="c113")
        assert 6 == len(df)
        assert "Lineitem_ID" in df.columns
        assert "Costcenter_ID" in df.columns

    def test_line_item_dataframe_single_doctype(self, populatedata, upload):
    ...
    def test_line_item_detailed_dataframe(self, populatedata, upload):
    ...
    def test_line_item_detailed_dataframe_single_doctype(self, populatedata, upload):
        df = LineItemManager().line_item_detailed_dataframe(doctype="co")
        assert 3 == len(df)
        assert "Lineitem_ID" in df.columns
        assert "Costcenter_ID" in df.columns

    def test_line_item_detailed_dataframe_single_fund(self, populatedata, upload):
